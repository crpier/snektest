...
from snektest import aload_fixture, load_fixture, test
...
@test()
async def test_fixture_without_teardown() -> None:
...
async def fixture_with_teardown() -> AsyncGenerator[int]:
...
@test()
async def test_fixture_with_teardown() -> None:
    fixture_value = await aload_fixture(fixture_with_teardown)
    assert fixture_value == 1
    assert fixture_with_teardown_started_up is False
...
@test()
async def test_fixture_is_called_only_once() -> None:
