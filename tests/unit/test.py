...
@test(2, 3)
@test(3, 4)
def test_simple_params(a: int, b: int) -> None:
    assert a + b == a + b


@test([1, 2, 3], 7)
@test([2, 3, 4], 10)
def test_params_and_fixtures(lst: list[int], expected_sum: int) -> None:
...
@test()
def test_with_parametrized_fixture() -> None:
    result = load_fixture(my_parametrized_fixture)
    assert result >= 1 or result // 2 != 0


@test()
def test_with_2_parametrized_fixtures() -> None:
    result = load_fixture(my_parametrized_fixture)
...
@test(2)
@test(1)
def test_with_2_parametrized_fixtures_and_parametrized_test(test_param: int) -> None:
    result = load_fixture(my_parametrized_fixture)
    result_2 = load_fixture(second_parametrized_fixture)
